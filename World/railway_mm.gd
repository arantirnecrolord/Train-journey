...
@export var distance_spale = 1.0:
	set(value):
		distance_spale = value
		is_dirty = true
var is_dirty = false
...
func _update_multimesh():
	...
	var count = floor(path_length / distance_spale)
	
	var mm: MultiMesh = $MultiMeshInstance3D.multimesh
	mm.instance_count = count
	var offset = distance_spale/1.0
	
	for i in range(0, count):
		var curve_distance = offset + distance_spale * i
